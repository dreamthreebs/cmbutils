...
class MatchedFilter:
    ...
    def __init__(self, nside, lmax, beam, cl_tot, pol=False, name="default"):
        ...
        if len(cl_tot) < (lmax + 1):
            raise ValueError(
                "the length cl is smaller than lmax, try other input lmax!"
            )

        if pol:  # if True, use polarization beam
            self.bl = hp.gauss_beam(fwhm=np.deg2rad(beam) / 60, lmax=lmax, pol=True)[
                : lmax + 1, 1
            ]
        else:
        ...
        self.cl_tot = cl_tot[
            : lmax + 1
        ]  # The power spectrum at some frequency except point sources, including diffuse foreground, instrumental noise, CMB ...
