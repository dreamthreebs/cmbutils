...
class EBLeakageCorrection:
    def __init__(self, m, lmax, nside, mask, post_mask, method:str='cutqufitqu', check_res:bool=False, n_iter=3, slope_in=None):
    ...
    def zzr(self, lmax, nside):
        ...
        def linear_fitting():
            if self.slope_in is None:
                coeffs = np.polyfit(self.tmp_b, self.crt_b, 1)
                self.slope, intercept = coeffs
                print(f'{self.slope=}, {intercept=}')
            else:
                self.slope = self.slope_in
                print(f'slope is given and {self.slope=}')

            self.cln_b = self.crt_b - self.slope * self.tmp_b
    ...
    def fullqufitqu(self, lmax, nside):
        ...
        def linear_fitting():
            ...
            self.cln_full_qu = self.crt_full_qu - self.slope * self.tmp_full_qu
    ...
    def cutqufitqu(self, m, lmax, nside):
        ...
        def linear_fitting():
            ...
            self.cln_cut_qu = self.crt_cut_qu - self.slope * self.tmp_cut_qu
    ...
    def fit_iter_crt_tmp(self, lmax, nside, n_iter_crt=9, n_iter_tmp=9):
        ...
        def linear_fitting():
            ...
            self.slope, intercept = coeffs
            print(f'{self.slope=}, {intercept=}')
            self.cln_cut_qu = self.iter_crt_qu - self.slope * self.iter_tmp_qu
